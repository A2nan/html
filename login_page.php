...
if (isset($_POST["valider"])) {
    if (!empty($_POST['pseudo']) && !empty($_POST['mdp'])) {
        ...
        // Requête pour récupérer les données de l'administrateur en fonction du pseudo
        $query = $db->prepare("SELECT password FROM admin WHERE pseudo = :pseudo");
        $query->bindParam(':pseudo', $pseudo_saisi);
        $query->execute();
        $data = $query->fetch(PDO::FETCH_ASSOC);

        if ($data && password_verify($mdp_saisi, $data['password'])) {
            session_start();
